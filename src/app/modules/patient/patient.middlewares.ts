...
export const updateMyPatientProfileMiddleware = (req : Request, res : Response, next : NextFunction) => { 
    ...
    const payload: IUpdatePatientProfilePayload = req.body;// object  { reportId: "123", shouldDelete: true },   
    ...
    if (files?.medicalReports && files?.medicalReports.length > 0) {
        ...
        if (payload.medicalReports && Array.isArray(payload.medicalReports)) {
            payload.medicalReports = [...payload.medicalReports, ...newReports]   /* after marge
             medicalReports: [
        { reportId: "123", shouldDelete: true },                    
        { reportName: "blood.pdf", reportLink: "cloudinary/..." },  
        { reportName: "xray.pdf",  reportLink: "cloudinary/..." },  
    ] */
        } else {
