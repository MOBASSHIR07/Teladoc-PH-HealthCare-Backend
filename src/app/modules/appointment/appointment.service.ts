...
import { IBookAppointmentPayload } from "./appointment.validation"
...
const bookAppointment = async (payload:IBookAppointmentPayload, user:JwtPayload) => {
 ...
 const patientData = await prisma.patient.findUniqueOrThrow({
   ...
   const doctorSchedules = await prisma.doctorSchedules.findUniqueOrThrow({
    where : {
        doctorId_scheduleId:{ // composite primary key ths why this way mixed
...
const getMyAppointments = async (user: JwtPayload) => {
...
const changeAppointmentStatus = async (appointmentId: string, appointmentStatus: AppointmentStatus, user: JwtPayload) => {
...
const getMySingleAppointment = async (user: JwtPayload, appointmentId: string) => {
      const patientData = await prisma.patient.findUnique({
        where: {
            email: user?.email
        }
...
const bookAppointmentWithPayLater = async (payload : IBookAppointmentPayload, user : JwtPayload) => {
    const patientData = await prisma.patient.findUniqueOrThrow({
        where: {
            email: user.email,
        }
...
const initiatePayment = async (appointmentId: string, user : JwtPayload) => {
    const patientData = await prisma.patient.findUniqueOrThrow({
        where: {
            email: user.email,
        }
