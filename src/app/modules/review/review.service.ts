...
const giveReview = async (user : JwtPayload, payload : ICreateReviewPayload) => {
...
const updateReview = async (user: JwtPayload, reviewId: string, payload: IUpdateReviewPayload) => {
    const patientData = await prisma.patient.findUniqueOrThrow({
        where: {
            email: user?.email
        }
