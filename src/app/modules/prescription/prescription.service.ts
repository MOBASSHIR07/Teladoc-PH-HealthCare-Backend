...
const givePrescription = async (user : JwtPayload, payload : ICreatePrescriptionPayload) => {
   ...
   }, { // default within 5 section alll transction should done , in this case it take more time so we are extending it
    maxWait : 15000,
    timeout: 20000,
   });
...
const myPrescriptions = async (user: JwtPayload) => {
...
const updatePrescription = async (user: JwtPayload, prescriptionId: string, payload: any) => {
    // Verify user exists
    const isUserExists = await prisma.user.findUnique({
        where: {
            email: user?.email
...
const deletePrescription = async (user: JwtPayload, prescriptionId: string): Promise<void> => {
    // Verify user exists
    const isUserExists = await prisma.user.findUnique({
        where: {
            email: user?.email
