...
const deleteAdmin = async (id: string, user: any) => {
    ...
    if(isAdminExist.id === user.userId) {
        throw new AppError(status.FORBIDDEN, "You cannot delete your own admin user");
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        await tx.user.update({
            ...
            data: {
                isDeleted: true,
                deletedAt: new Date(),
                status: UserStatus.DELETED // Optional:  may also want to block the user
            },
        })
...
const changeUserStatus = async (user : JwtPayload, payload : IChangeUserStatusPayload ) => {
...
const changeUserRole = async (user : JwtPayload, payload : IChangeUserRolePayload) => {
    ...
    if(!userId || !role){
        throw new AppError(status.BAD_REQUEST, "Please provide userId and role");
    }
